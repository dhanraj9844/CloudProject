...
const HomeMediaCard = ({ file }) => {
    ...
    return (
        <>
            <Card key={file?._id} >
                <CardImage onClick={handleMenuClick} src={file.cloudinaryUrl}alt="file" />
                <Box>
                    <Menu
                        id="basic-menu"
                        anchorEl={anchorEl}
                    ...
                    >
                        <Item onClick={() => { window.open(`${file.cloudinaryUrl}`, '_blank'); handleMenuClose() }} ><Visibility /> {file.contentType.includes('image') ? `View` : file.contentType.includes('video') ? 'Watch Video' : 'Open'}</Item>
